...
$title = "Riwayat Transaksi - ThriftHub";
ob_start();
...
$sql = "SELECT p.id AS pesanan_id, pi.nama AS produk, p.tanggal, pi.jumlah, (pi.harga*pi.jumlah) AS total,
        ...
        WHERE p.user_id = ? AND p.diterima = 1
  ...
  <div class="container">
          ...
          <tbody>
            <?php $no = 1; while($r = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td><span class="badge bg-success"><?= $r['status_pembayaran'] ?></span></td>
                <td><span class="badge bg-success"><?= $r['status_pengiriman'] ?></span></td>
                <td><?= $r['metode'] ?></td>
                <td><?= htmlspecialchars($r['lokasi_pengiriman']) ?></td>
              </tr>
            <?php endwhile; ?>
          </tbody>
        </table>
    ...
    <?php else: ?>
      <div class="text-center py-5">
        ...
        <h5 class="mt-3 text-secondary">Belum ada transaksi selesai.</h5>
      </div>
    <?php endif; ?>
  </div>
</section>
